UNIX BUILD NOTES
====================
Some notes on how to build BitFlip Core in Unix.
...
memory available when compiling BitFlip Core. On systems with less, gcc can be
...
See the section "Disable-wallet mode" to build BitFlip Core without wallet.
...
If you want to build BitFlip-Qt, make sure that the required packages for Qt development
...
# Configure BitFlip Core to use our own-built instance of BDB
cd $BITFLIP_ROOT
./autogen.sh
...
As mentioned above, when maintaining portability of the wallet between the standard BitFlip Core distributions and independently built
...
with 4.8-built BitFlip Core is needed follow the steps under "Berkeley DB" above.
