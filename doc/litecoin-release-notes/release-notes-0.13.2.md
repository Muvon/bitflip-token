BitFlip Core version 0.13.2 is now available from:

  <https://download.bitflip.info/litecoin-0.13.2.1/>
...
ECDSA signatures inside BitFlip transactions now use validation using
...
Previous versions of BitFlip Core had their mempool limited by checking
a transaction's fees against the node's minimum relay fee. There was no
upper bound on the size of the mempool and attackers could send a large
number of transactions paying just slighly more than the default minimum
relay fee to crash nodes with relatively low RAM. A temporary workaround
for previous versions of BitFlip Core was to raise the default minimum
relay fee.

BitFlip Core 0.13.2 will have a strict maximum size on the mempool. The
...
BitFlip Core 0.13.2 also introduces new default policy limits on the length and
...
BitFlip Core has a heuristic 'priority' based on coin value and age. This
calculation is used for relaying of transactions which do not pay the
minimum relay fee, and can be used as an alternative way of sorting
transactions for mined blocks. BitFlip Core will relay transactions with
...
entirely in BitFlip Core 0.13, and it is at this time undecided whether the
...
BitFlip Core has been updated to make use of this.

This means that if Tor is running (and proper authorization is available),
BitFlip Core automatically creates a hidden service to listen on, without
manual configuration. BitFlip Core will also use Tor automatically to connect
to other .onion nodes if the control socket can be successfully opened. This
will positively affect the number of available .onion nodes and their usage.

This new feature is enabled by default if BitFlip Core is listening, and
...
BitFlipd can now (optionally) asynchronously notify clients through a
...
(or `settxfee <n>` rpc during runtime). A value of `n=0` signals BitFlip
Core to use floating fees. By default, BitFlip Core will use floating
fees.
...
(default: `0.0002` FLIP/kB).

At all times, BitFlip Core will cap fees at `-maxtxfee=<x>` (default:
0.10) FLIP.
Furthermore, BitFlip Core will never create transactions paying less than
...
Various code modernizations have been done. The BitFlip Core code base has
...
for a new opcode in the BitFlip scripting system that in combination with
...
BitFlip transactions currently may specify a locktime indicating when
...
HD wallets are incompatible with older versions of BitFlip Core.
...
The two phases are distinct in the BitFlip-Qt GUI. During the first one,
...
- **Elimination of unwanted transaction malleability:** Segregating the witness
  ...
  malleability, which is a problem that makes programming BitFlip wallet
  software more difficult and which seriously complicates the design of smart
  contracts for BitFlip.
...
- **Weighting data based on how it affects node performance:** Some parts of
  each BitFlip block need to be stored by nodes in order to validate future
...
- **Increased security for multisig:** BitFlip addresses (both P2PKH addresses
  ...
  believed to be provided by BitFlip's choice of parameters for its Elliptic
...
- **More efficient almost-full-node security** Satoshi Nakamoto's original
  ...
  produce an accurate copy of BitFlip's current ledger (called the UTXO set),
  ...
  note that BitFlip Core does not provide an option to use this capability as
  of this 0.13.2 release.

- **Script versioning:** Segwit makes it easy for future soft forks to allow
  BitFlip users to individually opt-in to almost any change in the BitFlip
  Script language when those users receive new transactions.  Features
  currently being researched by Bitcoin and BitFlip Core contributors that may
...
segwit's start date of 1 January 2017 miners can update the BitFlip
client to BitFlip Core 0.13.2 to signal for segwit support. When a
...
Since BitFlip Core 0.10.0, nodes have defaulted to only relaying and
